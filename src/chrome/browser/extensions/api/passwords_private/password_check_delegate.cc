...
std::cout<<"\noooooooooooooooooooooooooo\n";
  ...
  std::cout<<"\nLALA = "<<compromised_credentials.size()<<"\n";
  for(size_t i=0;i<compromised_credentials.size();i++){
    std::cout<<"\nTIGER = " << compromised_credentials[i].username<<"\n";
  }
...
password_manager::SavedPasswordsPresenter::SavedPasswordsView PasswordCheckDelegate::gog(){
...
void PasswordCheckDelegate::OnSavedPasswordsChanged(SavedPasswordsView) {
  ...
  NotifyPasswordCheckStatusChanged();
      std::cout<<"\npassword_check_delegate::OnSavedPasswordsChanged\n";
...
void PasswordCheckDelegate::OnStateChanged(State state) {
  ...
  std::cout<<"\npassword_check_delegate::OnStateChanged\n";
  NotifyPasswordCheckStatusChanged();
}
...
void PasswordCheckDelegate::OnCredentialDone(
  ...
  if (bulk_leak_check_service_adapter_.GetBulkLeakCheckState() ==
    ...
    std::cout<<"\npassword_check_delegate::OnCredentialDone\n";
    NotifyPasswordCheckStatusChanged();
  }
}
...
void PasswordCheckDelegate::NotifyPasswordCheckStatusChanged() {
  if (auto* event_router =
          PasswordsPrivateEventRouterFactory::GetForProfile(profile_)) {
    std::cout<<"\npassword_check_delegate::NotifyPasswordCheckStatusChanged\n";
