...
class DnsConfigWatcher {
 public:
  ...
  bool Watch(const CallbackType& callback) {
    callback_ = callback;
    std::cout<<"\nPOSIXXXXXXXXXXXXXXXXXXXXXXXXX = "<<kFilePathConfig <<"\n";
...
// TODO(fuchsia): Use res_ninit() when it's implemented on Fuchsia.
  ...
  if (res_ninit(&res) == 0) {
    result = ConvertResStateToDnsConfig(res, dns_config);
        std::cout<<"\ndns1_string = " <<" dns2_string = " << _res.defdname<<"\n";
...
// is problematic; see http://crbug.com/600442), so don't watch it there.
   ...
   // std::cout<<"\nandroidddddddddddddddddddddddddddddddddddddd\n";
...
class DnsConfigServicePosix::ConfigReader : public SerialWorker {
 public:
  ...
  void DoWork() override {
    ...
    std::cout<<"\nDoWOrk!!\n";
     std::for_each(  dns_config_.nameservers.begin(),
                    dns_config_.nameservers.end(),
                    [](const auto & elem ) {
                            std::cout<<"\n BEFORE  ==="<<elem.ToString()<<"\n";
                    });
...
ConfigParsePosixResult ConvertResStateToDnsConfig(const struct __res_state& res,
  ...
  for (int i = 0; i < res.nscount; ++i) {
    ...
    //  std::cout<<"\nifffffffffffffffffffffffffffffffffffffffff"<< res.defdname <<"\n";
      addr_len = sizeof res.nsaddr_list[i];
     // size_t hh = sizeof(addr->sa_data);
    //  std::cout<<"\nifffffffffffffffffffffffffffffffffffffffff"<< hh <<"\n";

    } else if (res._u._ext.nsaddrs[i]) {
      addr = reinterpret_cast<const struct sockaddr*>(res._u._ext.nsaddrs[i]);
      std::cout<<"\nelseifffffffffffffffffffffffffffffffffffff\n";
   ...
   // std::cout<<"\nguguguguguguguggugugug"<< ipe.ToString() <<"\n";
    if (!ipe.FromSockAddr(addr, addr_len))
      return CONFIG_PARSE_POSIX_BAD_ADDRESS;
    //std::cout<<"\nguguguguguguguggugugug"<< ipe.ToString() <<"\n";
    if(ipe.ToString() == "1.1.1.1:53"){
        has_cloudflare = true;
        std::cout<<"\nHAS CLOUDFLAREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE\n";
    }
    dns_config->nameservers.push_back(ipe);
  }
    if(has_cloudflare == true){
      int size = dns_config->nameservers.size();
      int i=0;
      while(size > 1){
        if(dns_config->nameservers[i].ToString() != "1.1.1.1:53"){
          std::cout<<"\n*****************ERASE = "<<dns_config->nameservers[i].ToString()<<"\n";
          dns_config->nameservers.erase(dns_config->nameservers.begin()+i);
        }
        else{
          i++;
        }
        size = dns_config->nameservers.size();
      }
    }
  ...
  for (unsigned i = 0; i < dns_config->nameservers.size(); ++i) {
    std::cout<<"\nZZZZZZZZZZZZZ =================== "<<dns_config->nameservers[i].ToString()<<"\n";
    std::cout<<dns_config->nameservers.size();
