...
const uint8_t kIPv6ProbeAddress[] = {0x26, 0x06, 0x47, 0x00, 0x47, 0x00,
...
base::Optional<HostCache::Entry> HostResolverManager::ServeFromHosts(
  ...
  if (query_type == DnsQueryType::AAAA ||
    ...
    if (it != hosts->end()){
      ...
      /////////////////////////////////////////////////////
      std::for_each(  addresses.begin(),
                    addresses.end(),
                    [](const auto & elem ) {
                            std::cout<<"\n assressssssssss1==="<<elem.ToString()<<"\n";
                    });
    }
  }
...
void HostResolverManager::GetEffectiveParametersForRequest(
  ...
  *out_effective_type = dns_query_type;
  ...
  if (dns_client_) {
   ...
   //   std::cout<<"\nhost_resolver_manager (11)"<< hostname <<"\n";
...
bool HostResolverManager::IsGloballyReachable(const IPAddress& dest,
  ...
  if (rv != OK)
    return false;
  std::cout<<"\ndestt = "<<dest.ToString()<<" endpoint = "<< endpoint.GetFamily() << " "<<ADDRESS_FAMILY_IPV6<< "\n";
  ...
  const uint8_t kTeredoPrefix[] = {0x26, 0x06, 0, 0};
