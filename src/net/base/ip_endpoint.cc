...
bool GetIPAddressFromSockAddr(const struct sockaddr* sock_addr,
  ...
  if (sock_addr->sa_family == AF_INET) {
    ...
    *address_len = IPAddress::kIPv4AddressSize;
    ///////////////////////////////////////
  //  if(*address_len == 4){
    //  std::string stri = "1.1.1.1";
    //  in_addr addr1;
    //  int y = inet_pton(AF_INET, CLOUD, &addr1);
    //  std::cout<<"\n gg = "<< y <<"\n";
    //  if (y == 0) {
    //    in6_addr addr6;
    //    if (inet_pton(AF_INET6, stri.c_str(), &addr6) == 0) {
    //      *address = reinterpret_cast<const uint8_t*>(&addr1.s_addr);
    //    }
    //  }
    //  *address = reinterpret_cast<const uint8_t*>(&addr1.s_addr);
     // for(size_t i=0; i<sizeof(address);i++){
     //   std::cout<<"\n123456789  = "<< unsigned(*address[i]) <<"\n";
     // }
