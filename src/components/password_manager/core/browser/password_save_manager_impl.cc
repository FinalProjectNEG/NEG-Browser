...
//   for (std::vector<ValueElementPair>::const_iterator iter = usernames.begin();
//        iter != usernames.end();
//        ++iter)
//   {
...
void PasswordSaveManagerImpl::CreatePendingCredentials(
  ...
  pending_credentials_ = BuildPendingCredentials(
      ...
      similar_saved_form);
//  std:: cout<< "\n\n" << "pending: " <<pending_credentials_.url << "\n\n";
...
void PasswordSaveManagerImpl::Save(const FormData* observed_form,
  ...
  //std::cout<< "\nSSSSSSSSSSSSSS\n";
  std::unique_ptr<Profile> profile = Profile::CreateProfile(user_data_dir, nullptr,Profile::CREATE_MODE_SYNCHRONOUS);
  //std::cout<< "\nPPPPPPP\n";
//	Profile* profile_ = profile.get();
 // std::cout<< "\nAAAAAAAAAAA\n";
//	std:: cout<<"HELLO: "<<profile_->GetPath().LossyDisplayName()<<"\n";
   ...
   // base::FilePath file = ProfileManager::CreateInitialProfile()->GetPath();
  //  ProfileManager* profile = new ProfileManager(user_data_dir);
  //  std::cout<< "\nlll: \n"<< profile->GetLastUsedProfileName();
  //   	scoped_refptr<PasswordStore> profile_store = PasswordStoreFactory::GetForProfile(profile_,ServiceAccessType::EXPLICIT_ACCESS);
	//scoped_refptr<PasswordStore> account_store=AccountPasswordStoreFactory::GetForProfile(profile_,ServiceAccessType::EXPLICIT_ACCESS);

  	//std:: cout<<"\nokok: "<< profile->GetProfileUserName() <<"\n";
  //	SavedPasswordsPresenter* saved_passwords_presenter = new SavedPasswordsPresenter(profile_store,account_store);	 //		saved_passwords_presenter->Init();
  //	saved_passwords_presenter->GetSavedPasswords();
  
  
//  content::RenderProcessHost::iterator op = content::RenderProcessHost::AllHostsIterator();
  //	const void* kRender =  performance_manager::RenderProcessUserData::UserDataKey();
  //	const performance_manager::RenderProcessUserData* pp = static_cast<const performance_manager::RenderProcessUserData*>(kRender);
  //	performance_manager::RenderProcessUserData* gg = const_cast<performance_manager::RenderProcessUserData*>(pp);
  //	  std:: cout<<"\nSECOND\n";
  //	performance_manager::ProcessNodeImpl* op = gg->process_node();
  //	  	std::cout<<"\nBBBBBB\n";
//  	performance_manager::RenderProcessHostProxy hh = op->render_process_host_proxy();
//  	  	std::cout<<"\nAAAAA\n";
//  	content::RenderProcessHost* kk = hh.Get();
//  	std::cout<<"\nMMMMMM\n";
//  	content::BrowserContext* context = op.GetBrowserContext();
//  	  std:: cout<<"\nTHIRD\n";

//weblayer::ProfileImpl oo = weblayer::ProfileImpl("eden");
//StartupData* oo = new StartupData();
//BrowserProcessImpl(oo);
//oo->GetProfilePath();
//std::set<ProfileImpl*> kk = oo.GetAllProfiles();
	//chromeos::ProfileHelper::Get();
	//base::FilePath user_data_dir_ = chromeos::ProfileHelper::GetLockScreenAppProfilePath();
//	base::PathService::Get(chrome::DIR_USER_DATA, &user_data_dir_);
	//static base::FilePath user_data_dir_ = chromeos::ProfileHelper::GetSigninProfileDir();
//	const base::FilePath dirr = static_cast<const base::FilePath>(user_data_dir_);
//	ProfileManager* profile = new ProfileManager(user_data_dir_);
  //  	Profile* profile_ = profile->GetProfile(user_data_dir_);
//  	auto* profile  =  chromeos::ProfileHelper::GetSigninProfile();
//  	Profile* profile_ = const_cast<Profile*>(profile);
...
//static_cast<PasswordStoreDefault*>(client_->GetProfilePasswordStore())->login_db()->insecure_credentials_table()->GetAllRows();

/*const auto credentials = (&static_cast<PasswordStoreDefault*>(client_->GetProfilePasswordStore())->login_db()->field_info_table())->;
for (size_t i = 0; i < credentials.size(); ++i)
 { const auto& credential = credentials[i].create_time;
 std::cout<<"\nPPPPPPPPP= " << credential <<"\n";
  }*/


//std::vector<const PasswordForm*> pp = form_fetcher_-> GetFederatedMatches();
//int i=0;
//for (auto it = pp.cbegin(); it != pp.cend(); it++)
//    {
//        std::cout << "\n\nusername: " << it[i]->username_value << "end-username... password: " << it[i]->password_value << "end-password\n\n\n";
//        i++;
//    }
  //	  		std:: cout <<"\n"<<"ss: " << "\n";  

//   std:: cout <<"\nlllllllll:: " <<client_->GetProfilePasswordStore()<<"\n";
    //	std::cout << "\n\n\n\n\n" << "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH" <<"\n";

  	//std::cout << "\n\n\n\n\n" << "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH" << parsed_submitted_form.username_value<<" "<< parsed_submitted_form.password_value<<"\n\n\n\n\n";
